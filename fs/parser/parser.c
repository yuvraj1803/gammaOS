...
char* path_get_current_unit_and_advance(char** __path){
    ...
    char* current_unit_str = (char*) kzalloc(sizeof(char) * FS_MAX_PATH_UNIT_LENGTH);
    ...
    char* temp_path = (char*) kzalloc(sizeof(char) * strlen(*__path));
    ...
    if(strlen(current_unit_str) > FS_MAX_PATH_UNIT_LENGTH){
        kpanic("[KERNEL ERROR]: Provided invalid path. Unable to parse!");
    }
...
struct path* parse_path(const char* _path){
    ...
    char drive;
    struct path* parsed_path = (struct path*) kzalloc(sizeof(struct path));
    char* __path = (char*) kzalloc(sizeof(char)* strlen(_path));
    ...
    struct path_unit* unit_head = (struct path_unit*) kzalloc(sizeof(struct path_unit));
    ...
    while((current_unit = path_get_current_unit_and_advance(&__path)) != 0x0){
        struct path_unit* path_current_unit = (struct path_unit*) kzalloc(sizeof(struct path_unit));
